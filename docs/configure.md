...
You can use command `!!MCDR reload config` or its short form `!!MCDR r cfg` to reload the config file when MCDR is running. Check the [here](command.md#hot-reloads) for more detail about hot reloads
...
Advance console switch powered by [prompt-toolkit](https://pypi.org/project/prompt-toolkit/)
...
A list of custom [server handler](customize/handler.md) classes. The classed need to be subclasses of `AbstractServerHandler`
...
A list of custom [info reactor](customize/reactor.md) classes to handle the info instance. The classed need to be subclasses of `AbstractInfoReactor`
