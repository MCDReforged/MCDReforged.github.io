...
Commands can be registered by MCDR itself and by plugins. This page only introduces about commands from MCDR. For plugin registered command, check [here](plugin_dev/command.md)
...
1. Reload the plugin with id `my_plugin`
1. Enable and load the disabled plugin with file name `another_plugin.py.disabled`. It has plugin id `another_plugin`
1. Unload the plugin with id `another_plugin`
1. Reload the plugin with file name `another_plugin.py`. Note that since this plugin is not loaded, you can only use file name to specify it
...
See [here](preference.md) for more information about MCDR preference
