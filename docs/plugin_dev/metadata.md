...
As a single `.py` file only plugin, the metadata of [solo plugin](plugin_format.md#solo-plugin) is declared in the global scope of the source file. It's a dict contains several key-value with the name `PLUGIN_METADATA`
...
The following section [Fields](#fields) will use metadata declared in python syntax as examples
...
See [here](basic.md#entrypoint) for entrypoint document
...
See [here](cli.md#name) for more information
...
See [here](cli.md#pack) for more information
